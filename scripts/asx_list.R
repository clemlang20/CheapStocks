...
percentage_change_function  <- function(stock = "AVZ.AX", start_date = '2021-02-01' ){

      stock_reference_price <-tq_get(stock,get = 'stock.prices') %>%
                              filter(date == start_date) %>%
                              select(close)  
  ###
                      reference_date <- as.Date(start_date) 
  ###                    
                      today <- if(format(Sys.Date(), "%u") > 5) {
                      today <- Sys.Date()-2
                      }else{
  ...
  ###
                      stock_today_price <-  tq_get(stock,get = 'stock.prices') %>%
                      filter(date == today) %>%
                      select(close)
  ###
                      change_in_Price <- stock_today_price - stock_reference_price
                      
                      percentage_change <- (stock_today_price - stock_reference_price)/stock_reference_price * 100
                      
                      print(paste("Real change in price is", change_in_Price))
                      #print(change_in_Price)
                      
                      print(paste("Percentage change in price is", percentage_change))
